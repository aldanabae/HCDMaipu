...
<?php 
	$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'employee_spotlight', 'taxonomy');
	if($has_sidebar ==  'left'){
...
<?php
	...
	while ( have_posts() ) : the_post(); ?>
			...
			<?php emd_get_template_part('employee-spotlight', 'taxonomy', str_replace("_","-",$queried_object->taxonomy . '-' . $post->post_type)); ?>
			</div>
                <?php endwhile; // end of the loop. ?>
<?php	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'employee_spotlight', 'taxonomy');
	if($has_navigation){	
		global $wp_query;
