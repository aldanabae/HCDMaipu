...
if (!class_exists('Employee_Spotlight')):
	...
	final class Employee_Spotlight {
		...
		private function define_constants() {
			define('EMPLOYEE_SPOTLIGHT_VERSION', '4.3.0');
		...
		private function includes() {
			...
			if (!class_exists('EDD_SL_Plugin_Updater')) {
				require_once EMPLOYEE_SPOTLIGHT_PLUGIN_DIR . 'assets/ext/edd/EDD_SL_Plugin_Updater.php';
			}
			if (!class_exists('Emd_License')) {
				require_once EMPLOYEE_SPOTLIGHT_PLUGIN_DIR . 'includes/admin/class-emd-license.php';
			}
			if (!function_exists('emd_show_license_page')) {
				require_once EMPLOYEE_SPOTLIGHT_PLUGIN_DIR . 'includes/admin/license-functions.php';
			}
			//the rest
